...
public:
	...
	void init(uint16_t dev_id, uint32_t frame_buf_base_addr)
	{
        ...
        printf("psConf->BaseAddress = 0x%x\n", psConf->BaseAddress);
...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
	...
	void configureRead(uint16_t h_res, uint16_t v_res)
	{
		...
		context_.ReadCfg.EnableCircularBuf = 0;
		...
		context_.ReadCfg.FrameStoreStartAddr[0] = addr + context_.ReadCfg.HoriSizeInput * context_.ReadCfg.VertSizeInput * 1;
	...
	void setReadBuffer(uint32_t addr)
	{
		XStatus status;
		context_.ReadCfg.FrameStoreStartAddr[0] = addr;
		status = XAxiVdma_DmaSetBufferAddr(&drv_inst_, XAXIVDMA_READ, context_.ReadCfg.FrameStoreStartAddr);
		if (XST_SUCCESS != status)
		{
			throw std::runtime_error(__FILE__ ":" LINE_STRING);
